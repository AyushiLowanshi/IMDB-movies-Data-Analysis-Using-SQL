...
WHERE country LIKE '%INDIA%' OR
	  country LIKE '%USA%' AND year = 2019;
...
WITH movie_genre_summary AS ( SELECT title, COUNT(DISTINCT genre) AS genre_count
							  FROM genre G JOIN movie M
							  ON G.movie_id = M.id
                              GROUP BY title)
                              
SELECT COUNT(DISTINCT title) AS single_genre_movie_count
...
FROM genre G LEFT JOIN movie M
	 ON G.movie_id = M.id
GROUP BY genre;
...
WITH top_prod AS (SELECT M.production_company, 
...
WHERE DATE_FORMAT(date_published, '%Y-%m') = '2017-03'
	  AND country = 'USA'
...
-- Checking null values of column height 
SELECT COUNT(*) AS height_nulls
FROM names
WHERE height IS NULL;

-- Checking null values of column date_of_birth 
SELECT COUNT(*) AS date_of_birth_nulls
FROM names
WHERE date_of_birth IS NULL;

-- Checking null values of column known_for_movies 
SELECT 
...
SELECT COUNT(CASE WHEN name IS NULL 
                  THEN id END ) AS name_nulls,
       COUNT(CASE WHEN height IS NULL 
                  THEN id END ) AS height_nulls,
	   COUNT(CASE WHEN date_of_birth IS NULL 
                  THEN id END ) AS date_of_birth_nulls,
	   COUNT(CASE WHEN known_for_movies IS NULL 
...
WITH actress_ratings AS (
            ...
            JOIN movie M
                 ON RM.movie_id = M.id
            WHERE category = 'actress' AND LOWER(country) LIKE '%india%' AND m.languages LIKE '%hindi%'
	...
	role_mapping AS a
        ...
        movie AS m
            ...
            ratings AS r
				...
				genre AS g
				ON m.id=g.movie_id
WHERE category = 'actZX ress' AND lower(g.genre) ='drama'
GROUP BY actress_name
)
SELECT *,
